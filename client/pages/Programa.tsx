...
export default function Programa() {
  const [actividades, setActividades] = useState<ActividadCalendar[] | null>(null);
  ...
  const actividadesToShow = actividades ?? [
    ...
    { aula: 'Aula 2', titulo: 'Logística inversa', disertante: 'Ing. Ricardo Sosa', inicio: '10:00', fin: '11:30', color: 'Aula 2' },
    { aula: 'Aula 2', titulo: 'Big Data en transporte', disertante: 'Lic. Paula Castro', inicio: '12:00', fin: '13:00', color: 'Aula 2' },
    { aula: 'Aula 3', titulo: 'Taller: Simulación de flotas', disertante: 'Ing. Tomás Vera', inicio: '10:00', fin: '12:00', color: 'Aula 3' },
    { aula: 'Aula 3', titulo: 'Tendencias en movilidad eléctrica', disertante: 'Dra. Lucía Benítez', inicio: '14:00', fin: '15:30', color: 'Aula 3' },
    { aula: 'Aula 4', titulo: 'Infraestructura inteligente', disertante: 'Arq. Mariana Díaz', inicio: '11:00', fin: '12:00', color: 'Aula 4' },
    { aula: 'Aula 4', titulo: 'Políticas públicas de transporte', disertante: 'Lic. Carla Méndez', inicio: '13:00', fin: '14:00', color: 'Aula 4' },
    ...
    <Layout>
      <div className="container mx-auto px-2 py-8">
        <div className="text-center mb-8">
          ...
          {loading && <div className="text-congress-cyan mt-4">Cargando agenda...</div>}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full border border-gray-300 bg-white">
            <thead>
              <tr>
                <th className="bg-gray-100 px-2 py-2 text-center font-bold border-b border-r border-gray-300 w-20">Horario</th>
                {AULAS.map((aula) => (
                  <th key={aula} className="bg-gray-100 px-2 py-2 text-center font-bold border-b border-r border-gray-300">{aula}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {HORARIOS.map((hora, rowIdx) => (
                <tr key={hora}>
                  <td className="border-b border-r border-gray-300 text-center align-middle font-mono text-congress-cyan text-lg bg-gray-50">{hora}</td>
                  {AULAS.map((aula) => {
                    ...
                    if (actividad) {
                      ...
                      const rowSpan = Math.round(getRowSpan(actividad.inicio, actividad.fin));
                      const color = AULA_COLORS[actividad.color] || AULA_COLORS['Auditorio'];
                      return (
                        <td
                          key={aula + hora}
                          rowSpan={rowSpan}
                          className="border-b border-r border-gray-300 align-top p-0"
                          style={{ background: color.bg, minWidth: 180, maxWidth: 260 }}
                        >
                          <div
                      ...
                      return (
                        <td key={aula + hora} className="border-b border-r border-gray-300 bg-white min-w-[180px] max-w-[260px] text-center text-gray-300">-</td>
                      );
                    }
                  })}
                </tr>
